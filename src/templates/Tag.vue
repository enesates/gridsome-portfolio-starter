<template>
  <Layout>
    <div class="container-inner mx-auto my-16">
      <h2 class="text-4xl font-bold mb-8 border-b">Etiket: {{ $page.tag.title }}</h2>
      ...
      <div v-for="post in $page.tag.belongsTo.edges" :key="post.node.id" class="post border-gray-400 border-b mb-12">
        ...
        <div class="text-copy-secondary mb-4">
          ...
          <span> &middot; </span>
          <span>{{ post.node.timeToRead }} dk okuma süresi</span>
        </div>
        ...
        <div class="mb-8">
          <g-link :to="post.node.path" class="font-bold">Yazıya git</g-link>
        </div>
      </div>
...
export default {
  metaInfo() {
    return {
      title: 'Etiket: ' + this.$page.tag.title
    }
  },
  components: {
    PaginationPosts
